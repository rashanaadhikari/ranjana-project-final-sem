...
// // Dynamic import for PropertyMap from the user dashboard components
// const PropertyMap = dynamic(() => import("@/app/(user)/user/dashboard/properties/add/components/PropertyMap"), { ssr: false });
...
//       // KEY: Admin properties are automatically active
...
//             <div className="bg-gray-50/80 p-5 rounded-[1.8rem] border border-gray-100 flex flex-col gap-1 shadow-sm group hover:border-brand/20 transition-all">
//               <label className="text-[10px] font-black text-subtitle uppercase tracking-[0.25em] ml-1 opacity-60">Latitude</label>
//               <input
//                 type="number"
//                 step="any"
//                 placeholder="26.6666"
...
//                 className="bg-transparent border-none focus:outline-none focus:ring-0 text-base font-black text-heading mt-1"
//               />
//             </div>
//             <div className="bg-gray-50/80 p-5 rounded-[1.8rem] border border-gray-100 flex flex-col gap-1 shadow-sm group hover:border-brand/20 transition-all">
//               <label className="text-[10px] font-black text-subtitle uppercase tracking-[0.25em] ml-1 opacity-60">Longitude</label>
//               <input
//                 type="number"
//                 step="any"
//                 placeholder="87.2736"
...
export default function AdminPostPropertyForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // console.error("Submission failed:", error);
      setActiveToast({ message: errorMessage, type: "error" });
    } finally {
      setLoading(false);
    }
  };
