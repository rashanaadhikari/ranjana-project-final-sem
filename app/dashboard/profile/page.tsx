...
export default async function ProfilePage() {
  ...
  const name = metadata?.full_name || metadata?.name || user?.email?.split('@')[0] || "Guest User";
  const email = user?.email || "No email provided";
  
  // Fetch from DB if we want accurate data, but session might hold raw data
  // Using Mocks for Phone and Address if not on metadata
  const phone = metadata?.phone || "+977 98XXXXXXX";
